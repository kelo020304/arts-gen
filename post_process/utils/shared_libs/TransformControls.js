	...
	class TransformControlsGizmo extends THREE.Object3D {
		...
		constructor() {
			...
			const pickerTranslate = {
				X: [[ new THREE.Mesh( new THREE.CylinderGeometry( 0.3, 0, 1.3, 4, 1, false ), matInvisible ), [ 0.6, 0, 0 ], [ 0, 0, - Math.PI / 2 ]]],
				Y: [[ new THREE.Mesh( new THREE.CylinderGeometry( 0.3, 0, 1.3, 4, 1, false ), matInvisible ), [ 0, 0.6, 0 ]]],
				Z: [[ new THREE.Mesh( new THREE.CylinderGeometry( 0.3, 0, 1.3, 4, 1, false ), matInvisible ), [ 0, 0, 0.6 ], [ Math.PI / 2, 0, 0 ]]],
		...
		updateMatrixWorld( force ) {
			...
			for ( let i = 0; i < handles.length; i ++ ) {
				...
				const isTranslatePickerHandle = this.mode === 'translate' && handle.parent === this.picker[ 'translate' ];
				...
				if ( this.mode === 'translate' || this.mode === 'scale' ) {
					...
					if ( handle.name === 'X' || handle.name === 'XYZX' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitX ).applyQuaternion( quaternion ).dot( this.eye ) ) > AXIS_HIDE_TRESHOLD ) {
					...
					if ( handle.name === 'Y' || handle.name === 'XYZY' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitY ).applyQuaternion( quaternion ).dot( this.eye ) ) > AXIS_HIDE_TRESHOLD ) {
					...
					if ( handle.name === 'Z' || handle.name === 'XYZZ' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitZ ).applyQuaternion( quaternion ).dot( this.eye ) ) > AXIS_HIDE_TRESHOLD ) {
					...
					if ( handle.name === 'XY' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitZ ).applyQuaternion( quaternion ).dot( this.eye ) ) < PLANE_HIDE_TRESHOLD ) {
					...
					if ( handle.name === 'YZ' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitX ).applyQuaternion( quaternion ).dot( this.eye ) ) < PLANE_HIDE_TRESHOLD ) {
					...
					if ( handle.name === 'XZ' ) {
						...
						if ( ! isTranslatePickerHandle && Math.abs( _alignVector.copy( _unitY ).applyQuaternion( quaternion ).dot( this.eye ) ) < PLANE_HIDE_TRESHOLD ) {
