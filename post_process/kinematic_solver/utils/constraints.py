...
class CollisionConstraint:
    ...
    use_exact_mesh: bool = False

    def __call__(self) -> bool:
        return self.check()

    def calibrate_at_zero(self) -> bool:
        self.initial_overlap_pairs = self._current_overlap_pairs()
        return (
            self.config.allow_initial_penetration
            or not self.initial_overlap_pairs
        )

    def check(self) -> bool:
        current_pairs = set(self._current_overlap_pairs())
        if self.config.allow_initial_penetration:
            baseline_pairs = set(self.initial_overlap_pairs or [])
            return not (current_pairs - baseline_pairs)
        return not current_pairs

    def _current_overlap_pairs(self) -> list[tuple[str, str]]:
        if self.use_exact_mesh:
            return list(self.backend.exact_overlapping_pairs(self.moving_parts, self.static_parts))
        if hasattr(self.backend, "overlapping_pairs"):
            return list(self.backend.overlapping_pairs(self.moving_parts, self.static_parts))
        return [
            (moving, static)
