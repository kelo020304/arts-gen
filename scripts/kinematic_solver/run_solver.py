    ...
    try:
        with pred_path.open("w") as pred_out, trace_path.open("w") as trace_out:
            for joint_name, joint in sorted(oracle["joints"].items()):
                ...
                if enable_retained_overlap and joint["type"] == "prismatic":
                ...
                prediction = estimate_range(joint, evaluator)
                trace = {
                    "object_id": object_id,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable experimental moving-part retained-overlap constraint.",
    )
    parser.add_argument(
        "--write-visualization",
        action="store_true",
