...
def run_pipeline(
    *,
    ...
    common = [
        sys.executable,
        "-m",
        "scripts.kinematic_solver.data_prep",
    ...
    solver_cmd = [
        sys.executable,
        "-m",
        "scripts.kinematic_solver.run_solver",
        "--converter-output-root",
        str(roots.converter_output_root),
        "--source-root",
        str(roots.source_root),
    ...
    if not skip_validate:
        _shell([
            sys.executable,
            "-m",
            "scripts.kinematic_solver.run_validate",
            "--converter-output-root",
            str(roots.converter_output_root),
            "--source-root",
            str(roots.source_root),
    ...
    compare_cmd = [
        sys.executable,
        "-m",
        "scripts.kinematic_solver.run_compare",
        "--converter-output-root",
        str(roots.converter_output_root),
        "--source-root",
        str(roots.source_root),
    ...
    _shell([
        sys.executable,
        "-m",
        "scripts.kinematic_solver.report_summary",
        "--run-output-dir",
        str(run_output_dir),
        "--object-ids",
        ",".join(object_ids),
