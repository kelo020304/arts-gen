...
_ddp_common_normalize_gpu_ids() {
    local spec="$1"
    if [[ "$spec" =~ ^[0-9]+-[0-9]+$ ]]; then
        local start="${spec%-*}"
        local end="${spec#*-}"
        if [ "$start" -gt "$end" ]; then
            echo "[ERROR] Invalid GPU_IDS range: $spec" >&2
            exit 2
        fi
        local ids=()
        local i
...
launch_ddp() {
    ...
    if [ -n "$GPU_IDS" ]; then
        GPU_IDS="$(_ddp_common_normalize_gpu_ids "$GPU_IDS")"
    ...
    echo "[ddp_common] NUM_GPUS=$NUM_GPUS  TORCH_HOME=$TORCH_HOME  MASTER_ADDR=$MASTER_ADDR  MASTER_PORT=$MASTER_PORT  GPU_IDS='${GPU_IDS:-<unset>}'"
