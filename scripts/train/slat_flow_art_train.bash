#!/usr/bin/env bash
# ============================================================
# SLat Flow Art (paper Stage 4) Training Launcher (Phase 09 D-14/D-15/D-16)
#
# Usage:
#   bash scripts/train/slat_flow_art_train.bash
#   MAX_STEPS=5 bash scripts/train/slat_flow_art_train.bash    # smoke test
#
# MODE switches the variant yaml (resolved against CONFIG_DIR):
#   full     -> slat_flow_art/mv_4view.yaml          (no LoRA, no anchor)
#   lora     -> slat_flow_art/mv_4view_lora.yaml     (LoRA on attn)
#   lora_ext -> slat_flow_art/mv_4view_lora_ext.yaml (LoRA + keep_trainable hybrid)
#   l2sp     -> slat_flow_art/mv_4view_l2sp.yaml     (full + L2-SP anchor)
#   smoke    -> slat_flow_art/smoke_test.yaml        (single-GPU / CPU smoke)
#
...
CONFIG_DIR="TRELLIS-arts/configs/arts/slat_flow_art"
...
if [ -z "$WANDB_NAME" ]; then
    WANDB_NAME="slat-flow-art-${MODE}-${NUM_GPUS}gpu"
fi
