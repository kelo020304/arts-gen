"""Render SAM3D GLB mesh previews with vertex colors in Blender."""
...
def center_and_scale() -> tuple[mathutils.Vector, float]:
    objs = mesh_objects()
    if not objs:
...
def render_glb(glb_path: Path, out_prefix: str) -> list[Path]:
    clean_scene()
    setup_scene()
...
def main() -> None:
    targets = [
        ("complete", RUN_DIR / "complete.glb"),
        ("body", RUN_DIR / "parts" / "body.glb"),
        ("part_00", RUN_DIR / "parts" / "part_00.glb"),
        ("part_01", RUN_DIR / "parts" / "part_01.glb"),
        ("part_02", RUN_DIR / "parts" / "part_02.glb"),
    ]
    for prefix, glb in targets:
        if not glb.is_file():
            raise FileNotFoundError(glb)
        render_glb(glb, prefix)
