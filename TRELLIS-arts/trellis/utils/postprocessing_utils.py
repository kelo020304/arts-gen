...
def bake_texture(
    ...
    if mode == 'fast':
        ...
        rastctx = utils3d.torch.RastContext(backend='cuda')
        for observation, mask_i, view, projection in tqdm(zip(observations, masks, views, projections), total=len(observations), disable=not verbose, desc='Texture baking (fast)'):
            with torch.no_grad():
                rast = utils3d.torch.rasterize_triangle_faces(
                    rastctx, vertices[None], faces, observation.shape[1], observation.shape[0], uv=uvs[None], view=view, projection=projection
                )
                uv_map = rast['uv'][0].detach().flip(0)
                mask = rast['mask'][0].detach().bool() & mask_i
