...
class Trainer:
    ...
    def _on_log(self, metrics: dict, step: int):
        """日志钩子，供 Mixin（如 WandbMixin）覆盖以接收训练指标。"""
        pass

    def _on_snapshot(self, sample_dir: str, step: int):
        """快照钩子，供 Mixin（如 WandbMixin）覆盖以处理采样图像。"""
        pass

    @staticmethod
    def _flatten_numeric_log(data, prefix=''):
        out = {}
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                out.update(Trainer._flatten_numeric_log(value, name))
            elif isinstance(value, (int, float, np.integer, np.floating)) and np.isfinite(value):
                out[name] = float(value)
        return out

    @staticmethod
    def _format_progress_metrics(logs):
        flat_logs = [Trainer._flatten_numeric_log(item) for item in logs]
        keys = ('loss/loss', 'loss/mse', 'status/lr', 'status/grad_norm', 'scale', 'log_scale')
        labels = {
            'loss/loss': 'loss',
            'loss/mse': 'mse',
            'status/lr': 'lr',
        ...
        columns = []
        for key in keys:
            values = [item[key] for item in flat_logs if key in item]
            if not values:
                continue
            value = float(np.mean(values))
            if key == 'status/lr':
                columns.append(f'{labels[key]}: {value:.3e}')
            elif key in {'scale', 'log_scale'}:
                columns.append(f'{labels[key]}: {value:.3f}')
            else:
                columns.append(f'{labels[key]}: {value:.6f}')
        return columns
    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master and self.step % self.i_print == 0:
                ...
                columns.extend(self._format_progress_metrics([item for _, item in log]))
            ...
            if self.step % self.i_sample == 0:
                self.snapshot()
                if self.is_master:
                    sample_dir = os.path.join(self.output_dir, 'samples', f'step{self.step:07d}')
                    self._on_snapshot(sample_dir, self.step)
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ...
                    self._on_log(log_show, self.step)
                    log = []
