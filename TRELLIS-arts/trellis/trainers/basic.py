...
class BasicTrainer(Trainer):
    ...
    def _fill_compatible_missing_state_dict(self, model, model_ckpt):
        current_state = model.state_dict()
        compatible_missing_keys = []
        for key in ("view_pose_proj.weight", "view_pose_proj.bias", "view_id_embedding.weight"):
            if key in current_state and key not in model_ckpt:
                model_ckpt[key] = current_state[key]
                compatible_missing_keys.append(key)
        return compatible_missing_keys

    def _load_model_state_dict_compat(self, model, model_ckpt):
        """
        Load model checkpoints while allowing zero-init camera pose parameters
        and random-init view-id embeddings to be absent from older SS-flow
        checkpoints.
        """
        compatible_missing_keys = self._fill_compatible_missing_state_dict(model, model_ckpt)
        missing, unexpected = model.load_state_dict(model_ckpt, strict=False)
        bad_missing = [key for key in missing if key not in compatible_missing_keys]
        if bad_missing or unexpected:
            raise RuntimeError(
                f"checkpoint load failed: missing={bad_missing[:20]} "
                f"unexpected={unexpected[:20]}"
            )
        if self.is_master and compatible_missing_keys:
            print(f" allowed missing compatible keys: {compatible_missing_keys}", end='')
    ...
    def load(self, load_dir, step=0):
        ...
        for name, model in self.models.items():
            ...
            self._load_model_state_dict_compat(model, model_ckpt)
        ...
        if self.is_master:
            for i, ema_rate in enumerate(self.ema_rate):
                ...
                for name, model in self.models.items():
                    ...
                    self._fill_compatible_missing_state_dict(model, ema_ckpt)
