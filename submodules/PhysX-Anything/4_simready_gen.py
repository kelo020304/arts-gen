...
def generate_mjcf(
    ...
    # ---- actuator: add motor for each controllable joint
    actuator_entries = []  # list of (joint_name, gear_str or None)
    for group_idx in range(1, len(jsondata['group_info'])):
        jtype = jsondata['group_info'][str(group_idx)][-1]
        if jtype == 'B':
            actuator_entries.append(("slide_" + str(group_idx), None))
        elif jtype == 'C':
            actuator_entries.append(("pivot_" + str(group_idx), None))
        elif jtype == 'D':
            # ball joint is 3-DoF; need 3 motors with orthogonal gear vectors
            ball_name = "ball_" + str(group_idx)
            actuator_entries.append((ball_name, "1 0 0 0 0 0"))
            actuator_entries.append((ball_name, "0 1 0 0 0 0"))
            actuator_entries.append((ball_name, "0 0 1 0 0 0"))
        elif jtype == 'CB':
            actuator_entries.append(("pivot_" + str(group_idx), None))
            actuator_entries.append(("slide_" + str(group_idx), None))
    if actuator_entries:
        actuator = ET.SubElement(mujoco, "actuator")
        for joint_name, gear in actuator_entries:
            attrib = {
                "joint": joint_name,
                "ctrlrange": "-1 1",
            }
            if gear is not None:
                attrib["gear"] = gear
            ET.SubElement(actuator, "motor", attrib=attrib)
...
if __name__ == '__main__':
    ...
    if args.image_name:
        ...
        if not os.path.exists(os.path.join(basepath, stem, 'objs')):
            raise FileNotFoundError(f"objs/ not found in {os.path.join(basepath, stem)}, run 3_split.py first")
        namelist = [stem]
    ...
    for filename in namelist:
        ...
        if os.path.exists(os.path.join(basepath,filename,'objs')):
            ...
            if args.image_name:
                raise FileNotFoundError(f"objs/ not found in {os.path.join(basepath,filename)}, run 3_split.py first")
